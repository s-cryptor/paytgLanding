...
const formObj: Omit<Product, '_id'> = {
  title: '',
  imgSrc: '',
  price: [0],
  weight: [''],
...
watch(
  () => [form.value.carbs, form.value.proteins, form.value.fats],
  () => {
    form.value.kcal = form.value.proteins * 4 + form.value.carbs * 4 + form.value.fats * 9
  }
)
</script>
...
<template>
  ...
  >
    <NForm ref="formRef" :label-width="80" :model="form" :rules="rules" size="medium">
      ...
      <NFormItem label="Изображение" path="imgSrc">
        <NUpload
    ...
    <template #footer>
      <NButton secondary type="primary" @click="createProduct">Создать</NButton>
    </template>
  </NModal>
</template>
